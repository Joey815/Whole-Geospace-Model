...
!       **********************************
    ...
    use misc_mod
    use grid_mod
    use waccmx_neutral_mod, only: waccmx_phi_final_frame_seen, &
        waccmx_phi_skip_madala_after_final_enabled
...
!    real :: vexbpphi(nnx,nny),vexbhphi(nnx,nny)
    ...
    logical :: skip_madala_after_final
    real, save :: last_phi(nnx,nny)
    logical, save :: last_phi_valid = .false.

    data idpinter / 1 /
    data ipcrit   / 1 /

    skip_madala_after_final = waccmx_phi_final_frame_seen() .and. &
        waccmx_phi_skip_madala_after_final_enabled()
    if (skip_madala_after_final) then
        if (last_phi_valid) then
            phi = last_phi
            print *, 'SAMI3_PHI_SKIP_MADALA_AFTER_FINAL active; returning cached phi'
        else
            print *, 'SAMI3_PHI_SKIP_MADALA_AFTER_FINAL active; returning zero phi'
            do j = 1,nny
                do i = 1,nnx
                    phi(i,j) = 0.
                enddo
...
!!$                pcppy  = (0.5 * ( hipcp_pot(i,j)   + hipcp_pot(i,jp1) ))
    ...
    !     zero gradient in y
        ...
        if ( lmadala .and. .not. skip_madala_after_final ) then
            call madala_sevp(a1,a2,a5,a4,a3,si,dphi,f11_lb,f11_ub)
        else if (skip_madala_after_final) then
            do j = 1,nnyt
                do i = 1,nnx+1
                    dphi(i,j) = dphi0(i,j)
                enddo
...
!*******************************************************
    ...
    use misc_mod
    use grid_mod
    use waccmx_neutral_mod, only: waccmx_online_phi_enabled, &
        waccmx_recv_phi_weimer_online

    real :: phi_weimer_real(nfp1,nlt+1)
    real :: phi_weimer_interp(nfp1,nlt+1)
    real :: phi_weimer(nnx,nny)
    real :: hrutw1,hrutw2
    logical :: use_online_phi_weimer

    data iread_weimer / 1 /

    use_online_phi_weimer = waccmx_online_phi_enabled()

!       read in weimer at first time

    if ( iread_weimer == 1 ) then
        if (use_online_phi_weimer) then
            call waccmx_recv_phi_weimer_online(phi_weimer_real,hrut,hrutw2)
