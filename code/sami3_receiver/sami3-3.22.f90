...
!     *******************************************
    ...
    use waccmx_neutral_mod, only: waccmx_online_init, waccmx_online_finalize, &
                                  waccmx_neutral_update_hours
...
! *****************************************************************************
    ...
    call mpi_init(ierr)
    sami3_comm = MPI_COMM_WORLD
    call mpi_comm_rank(sami3_comm, taskid, ierr)
    call mpi_comm_size(sami3_comm, numtasks, ierr)
    write(*,*)'taskid =',taskid
...
! the number of divisions in params
    ...
    if(taskid == 0) then
        if(numwork /= numworkers) then
            ...
            call mpi_abort(sami3_comm, nerrorcode, ierr)
...
!            read(212) vsiout
    ...
    ! this call has to be seen by the master and workers
        ...
        call mpi_bcast(hrinit,1,MPI_REAL,0,sami3_comm,ierr)
...
!        ntmmax  = min(maxstep, int( (hrmax-hrpr) / dthr ))
        ...
        do while ( tflag )
            ...
            do iwrk = 1,numworkers
                call mpi_iprobe(iwrk,10,sami3_comm,flagit10, &
                status,ierr)
                if (flagit10) then
                    icnt10 = icnt10 + 1
...
!!$                                xnuin2t(i,j,kk)    = xnuin2(i,j,k)
                ...
                sami3_comm, flagit1, status, ierr)
...
!******************** worker task *******************************************
    ...
    ! field line loop: actual run
        ...
        neutral_update_hours = waccmx_neutral_update_hours()
...
14          format(i4)
    ...
    ! buffer and send to the LEFT
            ...
            right, 0, sami3_comm, status, ierr)
...
!       average magnetic pole grid values (deni,Ti,Te)
        ...
        ! update neutrals
            ...
            if( tneut >= neutral_update_hours ) then
                do nll = 1,nl
...
!                print *,'to print',ntm,timem,tprnt,timep
    ...
    call waccmx_online_finalize()

    call MPI_BARRIER(sami3_comm,ierr)
...
!    use apex_module,only: apxparm ! (GL)
    ...
    use grid_mod
    use hardy_mod
    use waccmx_neutral_mod, only: waccmx_online_init
    ...
    integer :: status(MPI_STATUS_SIZE)
	    ...
	    logical :: use_existing_phi_weimer, use_online_phi_weimer, phi_weimer_exists
	    character(len=32) :: use_existing_phi_weimer_env, use_online_phi_weimer_env
...
! Some local variables
    ...
    namelist / go / maxstep,hrmax,dthr,hrpr,dt0, &
      ...
      tmax,euv_fac,lfism,lwaccmx_neutral,waccmx_neutral_prefix,&
      lwaccmx_neutral_online,waccmx_online_port_file
...
!    print *,'start bcast 0'
    ...
    call mpi_bcast(maxstep,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(hrmax,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(dthr,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(hrpr,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(dt0,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(rmin,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(altmin,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(fbar,8,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(f10p7,8,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(ap,8,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(iyear,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(nday,8,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(imonth_1,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(iday_1,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(imonth_2,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(iday_2,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(mmass,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(nion1,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(nion2,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(tvn0,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(tvexb0,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(ver,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(veh,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(vw,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(gams,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(gamp,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(alt_crit_avg,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(blat_max,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(blat_min,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(snn,nneut,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(stn,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(denmin,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(alt_crit,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(cqe,1,MPI_REAL,0,sami3_comm,ierr)
!    call mpi_bcast(plat,1,MPI_REAL,0,sami3_comm,ierr)
!    call mpi_bcast(plon,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(psmooth,1,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(hall,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(storm_ti,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(storm_tf,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(vexb_max,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(restart,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lmadala,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lcr,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lvs,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lweimer,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(decay_time,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(pcrit,1,MPI_REAL,0,sami3_comm,ierr)
    if ( .NOT. restart) &
      call mpi_bcast(hrinit,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(lhwm93,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lhwm14,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(anu_drag0,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(kp,8,MPI_INTEGER,0,sami3_comm,ierr)
    call mpi_bcast(xhardy,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(tphi,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(tmax,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(euv_fac,1,MPI_REAL,0,sami3_comm,ierr)
    call mpi_bcast(lfism,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(lwaccmx_neutral,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(waccmx_neutral_prefix,len(waccmx_neutral_prefix), &
                   MPI_CHARACTER,0,sami3_comm,ierr)
    call mpi_bcast(lwaccmx_neutral_online,1,MPI_LOGICAL,0,sami3_comm,ierr)
    call mpi_bcast(waccmx_online_port_file,len(waccmx_online_port_file), &
                   MPI_CHARACTER,0,sami3_comm,ierr)

!    print *,'end bcast 0'

    call waccmx_online_init()
...
!        enddo
	...
	! set up weimer by master: weimer grid / data
	    ...
	    if ( taskid == 0 .AND. lweimer) then
	      use_existing_phi_weimer = .false.
	      use_online_phi_weimer = .false.
	      call get_environment_variable('SAMI3_USE_EXISTING_PHI_WEIMER', &
	           use_existing_phi_weimer_env)
	      if (trim(use_existing_phi_weimer_env) == '1' .OR. &
	          trim(use_existing_phi_weimer_env) == 'true' .OR. &
	          trim(use_existing_phi_weimer_env) == 'TRUE') then
	        use_existing_phi_weimer = .true.
	      endif
	      call get_environment_variable('SAMI3_USE_ONLINE_PHI_WEIMER', &
	           use_online_phi_weimer_env)
	      if (trim(use_online_phi_weimer_env) == '1' .OR. &
	          trim(use_online_phi_weimer_env) == 'true' .OR. &
	          trim(use_online_phi_weimer_env) == 'TRUE') then
	        use_online_phi_weimer = .true.
	      endif
	      if (use_online_phi_weimer) then
	        print *,'SAMI3_USE_ONLINE_PHI_WEIMER: using online MPI phi_weimer payload'
	      else if (use_existing_phi_weimer) then
	        inquire(file='phi_weimer.inp', exist=phi_weimer_exists)
	        if (.NOT. phi_weimer_exists) then
	          print *,'SAMI3_USE_EXISTING_PHI_WEIMER requested but phi_weimer.inp is missing'
	          stop 73
	        endif
	        print *,'SAMI3_USE_EXISTING_PHI_WEIMER: using preexisting phi_weimer.inp'
	      else
