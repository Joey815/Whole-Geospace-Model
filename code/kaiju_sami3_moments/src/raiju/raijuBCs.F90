module raijuBCs
    ...
    subroutine applyMomentIngestion(Model, Grid, State, doMomentIngest)
        ...
        !$OMP private(i,j,s,fIdx,fm,vm,kT,etaBelow,tmp_kti,tmp_kte,tmp_tiote,eMin,tmp_D,tmp_P)
            ...
            do i=Grid%shGrid%isg,Grid%shGrid%ieg
                ...
                do fm=1,Model%nFluidIn
                    ...
                    if (Grid%spc(s)%spcType .eq. RAIJUHPLUS) then
                        tmp_tiote = Model%tiote
                        if (Model%doUseStateTioteForMomentIngest .and. State%tiote(i,j) > TINY) then
                            tmp_tiote = State%tiote(i,j)
                        endif
                        tmp_kti = kT / (1.0 + 1.0/tmp_tiote)
                        tmp_kte = kT / (1.0 + tmp_tiote)
