...
module volttypes
    ...
    type, extends(imagCoupler_T) :: raijuCoupler_T
        ...
        logical :: doSami3MomentsIngest = .false.
            !! If true, overwrite RAIJU input plasma moments from a SAMI3 moments HDF5 product
        logical :: sami3MomentsReported = .false.
            !! Print SAMI3 moments ingest diagnostics once per run
        character(len=strLen) :: sami3MomentsFile = "NONE"
            !! SAMI3 moments HDF5 product read during real-time RAIJU coupling
        character(len=strLen) :: sami3MomentsGroup = "/RaiCplMomentsOnly"
            !! HDF5 group containing Pavg/Davg/Pstd/Dstd/tiote
        real(rp) :: sami3AlphaPavg = 1.0_rp
            !! Blend weight for SAMI3 Pavg. 1 overwrites, 0 keeps Voltron/RAIJU original.
        real(rp) :: sami3AlphaDavg = 1.0_rp
            !! Blend weight for SAMI3 Davg. 1 overwrites, 0 keeps Voltron/RAIJU original.
        real(rp) :: sami3AlphaPstd = 1.0_rp
            !! Blend weight for SAMI3 Pstd. 1 overwrites, 0 keeps Voltron/RAIJU original.
        real(rp) :: sami3AlphaDstd = 1.0_rp
            !! Blend weight for SAMI3 Dstd. 1 overwrites, 0 keeps Voltron/RAIJU original.
        real(rp) :: sami3AlphaTiote = 1.0_rp
            !! Blend weight for SAMI3 tiote. 1 overwrites, 0 keeps Voltron/RAIJU original.
        real(rp) :: sami3DensityFloor = 0.0_rp
            !! Minimum Davg after SAMI3 ingest/blending [#/cc].
        real(rp) :: sami3PressureFloor = 0.0_rp
            !! Minimum Pavg after SAMI3 ingest/blending [nPa].
        real(rp) :: sami3TioteMin = 0.0_rp
            !! Minimum tiote after SAMI3 ingest/blending.
        real(rp) :: sami3TioteMax = 1.0e30_rp
            !! Maximum tiote after SAMI3 ingest/blending.
        logical :: sami3AbortOnNonfinite = .true.
            !! Abort if a SAMI3 input field contains NaN/Inf before blending.
    ...
    type :: voltApp_T
        ...
        logical  :: doGCM = .false.
        logical  :: doLocalWaccmxStub = .false.
        logical  :: doLocalWaccmxFile = .false.
        character(len=strLen) :: gcmBackend = "NONE"
        character(len=strLen) :: waccmxContractFile = "waccmx_stub_contract.txt"
        character(len=strLen) :: waccmxExchangeFile = "waccmx_stub_exchange.md"
        character(len=strLen) :: waccmxPackageFile = "waccmx_stub_forward_package.h5"
        character(len=strLen) :: waccmxFeedbackFile = "waccmx_stub_feedback_package.h5"
        real(rp) :: waccmxSigmaPFloor = 5.0_rp
        real(rp) :: waccmxSigmaHFloor = 2.0_rp
        real(rp) :: waccmxSigmaPOval  = 10.0_rp
        real(rp) :: waccmxSigmaHOval  = 6.0_rp
