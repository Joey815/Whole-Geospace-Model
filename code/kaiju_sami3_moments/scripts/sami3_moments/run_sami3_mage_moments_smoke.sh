...
VOLTRON_TEMPLATE="${VOLTRON_TEMPLATE:-${TREE_ROOT}/analysis/runtime_ingest_blend_20260524/sami3_moments_base_control.volt.Res.00000.h5}"
MAP_WEIGHTS_VOLTRON_PREFIX="${MAP_WEIGHTS_VOLTRON_PREFIX:-${TREE_ROOT}/analysis/sami3_to_raiju_weights_voltron_shell_l_mlt_20260524}"
DIAG_DSOVERB_VOLTRON_WEIGHTFILE_PREFIX="${DIAG_DSOVERB_VOLTRON_WEIGHTFILE_PREFIX:-${TREE_ROOT}/analysis/sami3_voltron_raiju_diag_stubpayload_nfluid1_ds_over_B_voltron_shell_l_mlt_20260524}"
...
"${PYTHON_BIN}" \
  "${TREE_ROOT}/scripts/sami3_moments/build_sami3_to_raiju_weights.py" \
  "${OUT_DSOVERB_PREFIX}.h5" \
  --out "${MAP_WEIGHTS_VOLTRON_PREFIX}" \
  --raicpl-template "${RAICPL_TEMPLATE}" \
  --voltron-template "${VOLTRON_TEMPLATE}" \
  --mapping-mode voltron_shell_l_mlt

"${PYTHON_BIN}" \
  "${TREE_ROOT}/scripts/sami3_moments/sami3_moments_to_raiju_diag.py" \
  "${OUT_DSOVERB_PREFIX}.h5" \
  --out "${DIAG_DSOVERB_VOLTRON_WEIGHTFILE_PREFIX}" \
  --n-fluid-in 1 \
  --mapping-mode weights \
  --mapping-weight-file "${MAP_WEIGHTS_VOLTRON_PREFIX}.h5"

"${PYTHON_BIN}" \
  "${TREE_ROOT}/scripts/sami3_moments/validate_sami3_mage_moments.py" \
  "${OUT_DSOVERB_PREFIX}.h5" \
  "${DIAG_DSOVERB_VOLTRON_WEIGHTFILE_PREFIX}.h5" \
  --n-fluid-in 1

"${PYTHON_BIN}" - "${DIAG_DSOVERB_LMLT_PREFIX}.h5" "${DIAG_DSOVERB_VOLTRON_WEIGHTFILE_PREFIX}.h5" <<'PY'
import sys
...
with h5py.File(left_path, "r") as left, h5py.File(right_path, "r") as right:
    ...
    for path in paths:
        ...
        item_max = float(np.max(np.abs(diff)))
        item_rel = float(np.max(np.abs(diff) / np.maximum(np.abs(left_arr), 1.0)))
        print("{0} Voltron-shell max_abs={1} max_rel={2}".format(path, item_max, item_rel))
        max_abs = max(max_abs, item_max)
        max_rel = max(max_rel, item_rel)
    if max_abs > 5.0e-3 and max_rel > 1.0e-6:
        raise SystemExit("Voltron-shell weight-file mapping diverges from inline l_mlt")
print("Voltron-shell weight-file comparison max_abs={0} max_rel={1}".format(max_abs, max_rel))
PY
