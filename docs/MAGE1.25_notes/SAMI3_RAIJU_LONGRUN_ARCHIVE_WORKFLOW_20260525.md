...
scripts/validate_sami3_raiju_mapping_product.py on the XML sami3Moments file
scripts/summarize_sami3_raiju_longrun.py --label <label>
```
...
recommended_<label>_summary.txt/json generated by the summarizer
validate_sami3_raiju_mapping_product.txt/json
slurm-*.out
sacct_<jobid>.txt
...
The mapping-product smoke was also tested with `--skip-summary` against
long900.  It parsed the `sami3Moments` XML file, validated:

```text
product_h5 = sami3_voltron_raiju_diag_stubpayload_nfluid1_ds_over_B_l_mlt_20260524.h5
mapping_mode = l_mlt
...
The HDF5 summary step requires the local h5py environment.  By default the
archiver uses:
